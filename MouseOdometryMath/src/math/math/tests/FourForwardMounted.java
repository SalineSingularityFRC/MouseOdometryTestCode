...
public class FourForwardMounted {
	...
	// TODO add code to stop accounting for a mouse getting disconected, not
	// getting an image, or glitching
	...
	public void straightSidewaysSensorMotionReverse() {
		...
		// double[] sensorInfo = {sensor_dx1, sensor_dy1, sensor_dx2,
		// sensor_dy2};
	 ...
	 * 
	 * Mouse m1 = new Mouse(Math.PI/4, 2.0, Math.PI/4); Mouse m2 = new
	 * Mouse((5*Math.PI)/4, 2.0, Math.PI/4);
	 * 
	 * //Sensor input array double[][] bArray = {{5.0}, {0.0}, {-5.0}, {0.0}};
	 * //Matrix b = new Matrix(bArray);
	 * 
	 * 
	 * OdometryMath math = new OdometryMath(); double[] result =
	 * math.calculateRobotMotion(new Mouse[] {m1,m2}, bArray);
	 * 
	 * assertEquals(5.0, result[0], Robot.TOLERANCE); assertEquals(0.0,
	 * result[1], Robot.TOLERANCE); assertEquals(0.0, result[2],
	 * Robot.TOLERANCE); }
	 * 
	 * @Test public void straightSidewaysRobotMotion1(){
	 * 
	 * Mouse m1 = new Mouse(Math.PI/4, 2.0, Math.PI/4); Mouse m2 = new
	 * Mouse((5*Math.PI)/4, 2.0, Math.PI/4);
	 * 
